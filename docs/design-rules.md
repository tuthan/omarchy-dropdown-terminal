...
| PD12 One interaction model; input and focus are explicit | Required | Overlay is non-focusable and click-through; any future pet/tab input region is exact and bounded. Helper locking guarantees one mutation across per-screen instances. | Keyboard/pointer/pass-through, held-key, focus-loss, multiple-screen duplicate, and model-refresh tests. |
