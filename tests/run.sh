...
assert_status "QML resolves the toplevel monitor from refreshed client data" grep -Fq 'return root.monitorFromClientPayload(toplevel)' "$root_dir/Service.qml"
...
assert_status "QML defers async Hyprland observation checks" grep -Fq 'function completeObservationCheck()' "$root_dir/Service.qml"
assert_status "QML schedules the deferred observation timer" grep -Fq 'id: observationRefreshTimer' "$root_dir/Service.qml"
assert_status "QML retries observation through cross-monitor animation" grep -Fq 'maxObservationCheckAttempts: 15' "$root_dir/Service.qml"
assert_status "helper waits for cross-monitor move to settle" grep -Fq 'wait_window_y "$terminal_address" "$top_margin"' "$root_dir/bin/omarchy-dropdown-terminal"
...
assert_status "phase 1 overlay is lazy when disabled" grep -Fq 'active: root.effectsEnabled' "$root_dir/BarWidget.qml"
...
assert_status "phase 1 geometry polling backs off when stable" grep -Fq 'stableGeometrySamples >= 4 ? 1000 : 250' "$root_dir/TerminalEffects.qml"
assert_status "phase 1 refreshes only on changed geometry" grep -Fq 'before !== root.toplevelGeometrySnapshot()' "$root_dir/Service.qml"
...
assert_status "phase 5 remaps progress by edge fraction on resize" grep -Fq 'fractionForProjection' "$root_dir/PetMotion.qml"
...
assert_status "phase 3 checks decoded atlas dimensions" grep -Fq 'atlas.sourceSize.width' "$root_dir/PetSprite.qml"
assert_status "phase 3 clips the single decoded atlas viewport" grep -Fq 'clip: true' "$root_dir/PetSprite.qml"
assert_false "phase 3 does not reload a source clip per frame" grep -Fq 'sourceClipRect' "$root_dir/PetSprite.qml"
assert_status "phase 3 exposes asset diagnostics" grep -Fq 'property string petDiagnostic' "$root_dir/PetLayer.qml"
...
assert_status "phase 3 pet layer owns the surface readiness gate" grep -Fq 'surfaceReady: root.surfaceReady' "$root_dir/PetLayer.qml"
assert_status "phase 3 effect and pet loaders are independent" grep -Fq 'id: petLoader' "$root_dir/BarWidget.qml"
assert_status "phase 3 scales alternate particles with intensity" grep -Fq 'variantBaseCount * root.intensity' "$root_dir/TerminalEffects.qml"
...
assert_status "phase 5 has a pure pixel route planner" test -s "$root_dir/PetRoute.js"
assert_status "phase 5 planner has bounded pixel steps" grep -Fq 'requestedDistance = stepCount * stride * stepScale' "$root_dir/PetRoute.js"
assert_status "phase 5 planner returns authored-duration travel" grep -Fq 'durationMs: Math.max(0, Math.round(travelDistance / speed * 1000))' "$root_dir/PetRoute.js"
assert_status "phase 5 motion has signed direction" grep -Fq 'property int direction' "$root_dir/PetMotion.qml"
assert_status "phase 5 motion exposes wall side" grep -Fq 'property string wallSide' "$root_dir/PetMotion.qml"
assert_status "phase 5 motion remaps by edge fraction" grep -Fq 'fractionForProjection' "$root_dir/PetMotion.qml"
assert_status "phase 5 route uses linear easing" grep -Fq 'easing.type: Easing.Linear' "$root_dir/PetController.qml"
assert_false "phase 5 route has no sine position easing" grep -Fq 'Easing.InOutSine' "$root_dir/PetController.qml"
assert_status "phase 5 has turn action plumbing" grep -Fq 'startTurnOrCorner' "$root_dir/PetController.qml"
assert_status "phase 5 has pet press generation cancellation" grep -Fq 'petPressGeneration' "$root_dir/PetController.qml"
assert_status "phase 5 has bounded hearts" grep -Fq 'model: root.burstKind === "hearts" ? 6 : 0' "$root_dir/PetEffects.qml"
assert_status "phase 5 subtracts the resize grab region" grep -Fq 'intersection: Intersection.Subtract' "$root_dir/PetLayer.qml"
assert_status "phase 5 input follows the sprite" grep -Fq 'item: root.interactionEnabled ? petController.spriteItem : null' "$root_dir/PetLayer.qml"
assert_status "phase 5 keeps keyboard focus disabled" grep -Fq 'WlrLayershell.keyboardFocus: WlrKeyboardFocus.None' "$root_dir/PetLayer.qml"
assert_status "phase 5 queries Hyprland grab options" grep -Fq 'general:border_size; hyprctl -j getoption general:extend_border_grab_area; hyprctl -j getoption general:resize_on_border' "$root_dir/Service.qml"
assert_status "phase 5 ignores inactive resize grab area" grep -Fq 'resizeOnBorder ? Math.max(0, values[0] + values[1]) : values[0]' "$root_dir/Service.qml"
assert_status "phase 5 has the interaction setting reader" grep -Fq 'readonly property bool petInteraction' "$root_dir/Service.qml"
assert_status "phase 5 has the roaming setting reader" grep -Fq 'readonly property string petRoaming' "$root_dir/Service.qml"
assert_status "phase 6/7 manifest version" jq -e '.version == "2.3.0"' "$root_dir/manifest.json"
assert_status "phase 5 manifest settings" jq -e '.barWidget.schema as $schema | any($schema[]; .key == "petInteraction" and .defaultValue == true) and any($schema[]; .key == "petRoaming" and .options == ["Top edge","Whole border"])' "$root_dir/manifest.json"
assert_status "phase 6 packs use manifest version 3" bash -c 'jq -e ".version == 3" "$1" "$2" "$3" >/dev/null' _ "$root_dir/assets/pets/penguin/pet.json" "$root_dir/assets/pets/cat/pet.json" "$root_dir/assets/pets/corgi/pet.json"
assert_status "phase 6 penguin has stride and happy/turn" jq -e '.version == 3 and .actions.walk.stride >= 4 and .actions.happy.loop == true and .actions.turn.loop == false' "$root_dir/assets/pets/penguin/pet.json"
assert_status "phase 5 planner tests exist" test -s "$root_dir/tests/qml/tst_pet_route.qml"
assert_status "phase 6 drag uses the MouseArea pressed state" grep -Fq 'petMouseArea.pressed' "$root_dir/PetController.qml"
assert_status "phase 6 remembered routes carry direction across edges" grep -Fq 'towardDirection' "$root_dir/PetController.qml"
assert_status "phase 6 staging land does not overwrite remembered position" grep -Fq '!root.returningToRemembered' "$root_dir/PetController.qml"
assert_status "phase 6 corner compensation uses the displayed corner anchor" grep -Fq 'projectedAnchorFor("corner", false)' "$root_dir/PetController.qml"
assert_status "phase 6 celebration cancellation preserves speech" bash -c 'awk '\''/function cancel\(\)/ { found=1; next } found && /dismissBubble/ { exit 1 } found && /^  }/ { exit 0 } END { if (!found) exit 1 }'\'' "$1"' _ "$root_dir/PetEffects.qml"
assert_status "phase 6 geometry remap clears drag reaction" grep -A8 -F 'function handleGeometryRemap' "$root_dir/PetController.qml" | grep -Fq 'reactionX = 0'
assert_status "phase 6 carried state is press-protected" grep -Fq '"carried"].indexOf(root.petState)' "$root_dir/PetController.qml"
assert_status "phase 6 hover timers use lexical QML ids" grep -Fq 'hoverDwellTimer.stop()' "$root_dir/PetController.qml"
assert_status "phase 6 side hitbox follows the wall anchor" grep -Fq 'projectedAnchorX' "$root_dir/PetController.qml"
qml_runner=""
for candidate in /usr/lib/qt6/bin/qmltestrunner "$(command -v qmltestrunner6 2>/dev/null || true)"; do
  if [[ -x "$candidate" ]] && ldd "$candidate" 2>/dev/null | grep -q 'libQt6'; then
    qml_runner="$candidate"
    break
  fi
done
if [[ -n "$qml_runner" ]]; then
  assert_status "phase 5/6 QML tests pass" env QT_QPA_PLATFORM=minimal QT_QPA_PLATFORMTHEME=none \
    "$qml_runner" -input "$root_dir/tests/qml"
fi
assert_status "phase 6 design rule revision is recorded" grep -Fq 'Phase 6 extends the Phase 5 profile row' "$root_dir/docs/design-rules.md"

# Phase 6 pet-world contract: authored posture families, bounded pointer/state
# behavior, and optional media remain separately testable before a compositor
# session is available.
assert_status "phase 6 has the route approach API" grep -Fq 'towardU' "$root_dir/PetRoute.js"
assert_status "phase 6 has allowed edge planning" grep -Fq 'allowedEdges' "$root_dir/PetRoute.js"
assert_status "phase 6 has nearest-edge drop projection" grep -Fq 'function nearestEdgePoint' "$root_dir/PetRoute.js"
assert_status "phase 6 has wall-step corner planning" grep -Fq 'input.wallStep === true' "$root_dir/PetRoute.js"
assert_status "phase 6 motion exposes per-edge room" grep -Fq 'readonly property real roomBottom' "$root_dir/PetMotion.qml"
assert_status "phase 6 motion receives atlas scale" grep -Fq 'renderScale: root.spriteItem ? root.spriteItem.renderScale' "$root_dir/PetController.qml"
assert_status "phase 6 controller uses posture families" grep -Fq 'function actionFamily(edge)' "$root_dir/PetController.qml"
assert_status "phase 6 controller compensates corner anchors" grep -Fq 'function applyAnchorCompensation(plan)' "$root_dir/PetController.qml"
assert_status "phase 6 controller supports drag" grep -Fq 'function finishDrag()' "$root_dir/PetController.qml"
assert_status "phase 6 drag snaps to allowed edge" grep -Fq 'PetRoute.nearestEdgePoint' "$root_dir/PetController.qml"
assert_status "phase 6 hover has a bounded dwell timer" grep -Fq 'id: hoverDwellTimer' "$root_dir/PetController.qml"
assert_status "phase 6 hover halo is an input union" grep -Fq 'root.hoverHaloExtent > 0' "$root_dir/PetLayer.qml"
assert_status "phase 6 halo cost is documented in the panel" grep -Fq 'clicks inside it are consumed by the pet layer' "$root_dir/Panel.qml"
assert_status "phase 6 has pet state atomic writes" grep -Fq 'atomicWrites: true' "$root_dir/Service.qml"
assert_status "phase 6 pet state uses one owner" grep -Fq 'petStateWriter' "$root_dir/Service.qml"
assert_status "phase 6 has the voice library" test -s "$root_dir/PetVoice.js"
assert_status "phase 6 has the voice asset validator" test -x "$root_dir/bin/omarchy-dropdown-terminal-voice-validate"
assert_status "phase 6 voice asset validates" "$root_dir/bin/omarchy-dropdown-terminal-voice-validate" validate "$root_dir/assets/voice/lines.json"
assert_status "phase 6 voice loader is opt-in" grep -Fq 'root.petVoice === "Off"' "$root_dir/PetController.qml"
assert_status "phase 6 voice bubble is noninteractive" grep -Fq 'enabled: false' "$root_dir/PetEffects.qml"
assert_status "phase 6 has the sound component" test -s "$root_dir/PetSound.qml"
assert_status "phase 6 sound loader is opt-in" grep -Fq 'root.petSound !== "Off"' "$root_dir/PetController.qml"
assert_status "phase 6 sound assets have Ogg signatures" bash -c 'for file in "$1"/*.ogg; do [[ "$(head -c 4 "$file")" == OggS ]]; done' _ "$root_dir/assets/sounds"
assert_status "phase 6 sound assets stay bounded" bash -c 'for file in "$1"/*.ogg; do (( $(stat -c %s "$file") <= 49152 )); done' _ "$root_dir/assets/sounds"
assert_status "phase 6 has all new settings" jq -e '.barWidget.schema as $schema | ["petDrag","petHoverHalo","petVoice","petSound","petRememberPosition"] | all(.[]; . as $key | any($schema[]; .key == $key))' "$root_dir/manifest.json"
assert_status "phase 6 packs expose descending climb and hang" bash -c 'jq -e ".version == 3 and .actions.climbDown and .actions.hang" "$1" >/dev/null && jq -e ".version == 3 and .actions.climbDown and .actions.hang" "$2" >/dev/null && jq -e ".version == 3 and .actions.climbDown and .actions.hang" "$3" >/dev/null' _ "$root_dir/assets/pets/penguin/pet.json" "$root_dir/assets/pets/cat/pet.json" "$root_dir/assets/pets/corgi/pet.json"
assert_status "phase 6 pet review sheet tool exists" test -x "$root_dir/bin/omarchy-dropdown-terminal-pet-sheet"
assert_status "phase 6 design gate records state inventory" grep -Fq 'Pet position' "$root_dir/docs/design-rules.md"
assert_status "phase 6 resize-grab rule is recorded in the design profile" grep -Fq 'general:border_size + general:extend_border_grab_area' "$root_dir/docs/design-rules.md"

# Phase 7 villain and bond contract: keep the new state machine, persistence,
# input union, and release assets independently auditable before a compositor
# session is available.
assert_status "phase 7 has pure bond arithmetic" test -s "$root_dir/PetBond.js"
assert_status "phase 7 has pure encounter arithmetic" test -s "$root_dir/PetEncounter.js"
assert_status "phase 7 has encounter surface" test -s "$root_dir/PetEncounter.qml"
assert_status "phase 7 encounter has no second layer surface" grep -Fq 'PanelWindow {' "$root_dir/PetLayer.qml" && assert_false "phase 7 encounter creates no PanelWindow" grep -Fq 'PanelWindow {' "$root_dir/PetEncounter.qml"
assert_status "phase 7 layer instantiates encounter" grep -Fq 'PetEncounter {' "$root_dir/PetLayer.qml"
assert_status "phase 7 villain joins input union" grep -Fq 'encounterInputEnabled' "$root_dir/PetLayer.qml"
assert_status "phase 7 encounter keeps resize subtraction" grep -Fq 'intersection: Intersection.Subtract' "$root_dir/PetLayer.qml"
assert_status "phase 7 encounter priority is explicit" grep -Fq '"encounter", "failure"' "$root_dir/PetController.qml"
assert_status "phase 7 controller owns encounter hand-off" grep -Fq 'function beginEncounter(' "$root_dir/PetController.qml"
assert_status "phase 7 controller routes success" grep -Fq 'successArrived' "$root_dir/PetController.qml"
assert_status "phase 7 controller interrupts encounter" grep -Fq 'encounterItem.interrupt()' "$root_dir/PetController.qml"
assert_status "phase 7 encounter has hard deadline" grep -Fq 'MAX_ENCOUNTER_MS' "$root_dir/PetEncounter.qml"
assert_status "phase 7 encounter uses towardU" grep -Fq 'towardU: root.standoffU' "$root_dir/PetEncounter.qml"
assert_status "phase 7 encounter uses reroll floor" grep -Fq 'rerollAllowed' "$root_dir/PetEncounter.qml"
assert_status "phase 7 bond path is state-scoped" grep -Fq 'io.github.tuthan.dropdown-terminal' "$root_dir/Service.qml" && assert_status "phase 7 bond filename is explicit" grep -Fq 'bond.json' "$root_dir/Service.qml"
assert_status "phase 7 bond uses atomic writes" grep -A5 -F 'id: bondFile' "$root_dir/Service.qml" | grep -Fq 'atomicWrites: true'
assert_status "phase 7 bond has one owner" grep -Fq 'bondOwner' "$root_dir/Service.qml"
assert_status "phase 7 bond uses delta merge" grep -Fq 'PetBond.applyDeltas' "$root_dir/Service.qml"
assert_status "phase 7 bond retries external changes" grep -Fq 'bondFlushRetry' "$root_dir/Service.qml"
assert_status "phase 7 exposes villain setting" jq -e '.barWidget.schema | any(.[]; .key == "petVillains" and .defaultValue == true)' "$root_dir/manifest.json"
assert_status "phase 7 exposes playful activity" jq -e '.barWidget.schema[] | select(.key == "petActivity") | .options | index("Playful") != null' "$root_dir/manifest.json"
assert_status "phase 7 playful activity uses a shorter timer" grep -Fq '1200 + Math.floor(root.nextRandom() * 2401)' "$root_dir/PetController.qml"
assert_status "phase 7 playful activity scales route steps" grep -Fq 'stepScale: root.activity === "Playful" ? 3 : 1' "$root_dir/PetController.qml"
assert_status "phase 7 panel exposes villain toggle" grep -Fq 'text: (on ? "✓ " : "") + "Villains"' "$root_dir/Panel.qml"
assert_status "phase 7 panel exposes bond track" grep -Fq 'Friendship unlocks more lines:' "$root_dir/Panel.qml"
assert_status "phase 7 panel exposes reset bond" grep -Fq 'text: "Reset bond"' "$root_dir/Panel.qml"
assert_status "phase 7 panel explains tracking diagnostic" grep -Fq 'Villains need command tracking' "$root_dir/Panel.qml"
assert_status "phase 7 voice events are known" grep -Fq 'villainAppear' "$root_dir/PetVoice.js"
assert_status "phase 7 voice event budget is bounded" grep -Fq '< 3' "$root_dir/PetVoice.js"
assert_status "phase 7 puff effect is bounded" grep -Fq 'burstKind === "puff" ? 4' "$root_dir/PetEffects.qml"
assert_status "phase 7 sound cues are selectable" grep -Fq '"villain", "victory"' "$root_dir/PetSound.qml"
assert_status "phase 7 bug validator passes" "$root_dir/bin/omarchy-dropdown-terminal-pet-validate" validate villain "$root_dir/assets/villains/bug"
assert_status "phase 7 ghost validator passes" "$root_dir/bin/omarchy-dropdown-terminal-pet-validate" validate villain "$root_dir/assets/villains/ghost"
assert_status "phase 7 villain atlases are exact size" bash -c 'identify -format "%wx%h" "$1/villain.png" | grep -Fxq "256x128" && identify -format "%wx%h" "$2/villain.png" | grep -Fxq "256x128"' _ "$root_dir/assets/villains/bug" "$root_dir/assets/villains/ghost"
assert_status "phase 7 new sound assets have Ogg signatures" bash -c '[[ "$(head -c 4 "$1/villain.ogg")" == OggS ]] && [[ "$(head -c 4 "$1/victory.ogg")" == OggS ]]' _ "$root_dir/assets/sounds"
assert_status "phase 7 design rule revision is recorded" grep -Fq 'villain rectangle' "$root_dir/docs/design-rules.md"

voice_bad="$runtime_dir/voice-bad"
cp "$root_dir/assets/voice/lines.json" "$voice_bad"
jq '.tiers.Kind.failed[0] = "$HOME"' "$voice_bad" > "$runtime_dir/voice-bad.json"
mv "$runtime_dir/voice-bad.json" "$voice_bad"
assert_false "voice validator rejects shell expansion text" "$root_dir/bin/omarchy-dropdown-terminal-voice-validate" validate "$voice_bad"

pet_bad_kind="$runtime_dir/pet-bad-kind"
cp -a "$root_dir/assets/pets/cat" "$pet_bad_kind"
jq '.kind = "villain"' "$pet_bad_kind/pet.json" > "$runtime_dir/pet-bad-kind.json"
mv "$runtime_dir/pet-bad-kind.json" "$pet_bad_kind/pet.json"
assert_false "pet validator rejects villain kind" "$root_dir/bin/omarchy-dropdown-terminal-pet-validate" validate "$pet_bad_kind"
...
villain_bad="$runtime_dir/villain-bad"
cp -a "$root_dir/assets/villains/bug" "$villain_bad"
jq '.atlas.path = "../villain.png"' "$villain_bad/villain.json" > "$runtime_dir/villain-bad-path.json"
mv "$runtime_dir/villain-bad-path.json" "$villain_bad/villain.json"
assert_false "villain validator rejects traversal path" "$root_dir/bin/omarchy-dropdown-terminal-pet-validate" validate villain "$villain_bad"
rm -rf "$villain_bad"
cp -a "$root_dir/assets/villains/bug" "$villain_bad"
rm "$villain_bad/villain.png"
ln -s LICENSE "$villain_bad/villain.png"
assert_false "villain validator rejects symlink" "$root_dir/bin/omarchy-dropdown-terminal-pet-validate" validate villain "$villain_bad"
rm -rf "$villain_bad"
cp -a "$root_dir/assets/villains/bug" "$villain_bad"
chmod +x "$villain_bad/villain.png"
assert_false "villain validator rejects executable content" "$root_dir/bin/omarchy-dropdown-terminal-pet-validate" validate villain "$villain_bad"
...
assert_status "bar widget exposes panel open state" grep -Fq 'property bool opened: settingsLoader.item' "$root_dir/BarWidget.qml"
...
assert_status "helper rotates the bounded command journal" grep -Fq 'rotate-events' "$root_dir/bin/omarchy-dropdown-terminal"
assert_status "command reducer uses null-prototype maps" grep -Fq 'Object.create(null)' "$root_dir/Service.qml"
assert_status "command reducer isolates malformed lines" grep -Fq 'event record rejected' "$root_dir/Service.qml"
